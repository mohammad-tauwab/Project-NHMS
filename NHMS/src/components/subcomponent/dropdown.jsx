...
function DropDown({
  ...
  const handleCheckBoxChange = (checkedItem) => {
    setSelectedItem((currentItemList) => {
      return currentItemList.includes(checkedItem)
        ? currentItemList.filter((items) => {
            return items != checkedItem;
          })
        : [...currentItemList, checkedItem];
    });
  };
  ...
  return (
    <div className="my-2 relative max-h-fit mx-3 w-[80%]">
      <div
        className="flex flex-row border-[1px] border-slate-300 bg-slate-200 rounded-xl px-3 py-1 hover:border-black hover:bg-blue-200 cursor-pointer m-auto"
        onClick={toggleDropDown}
      >
        <input
          type="button"
          value={`${isOpen ? "Done " : buttonName}`}
          className="cursor-pointer text-slate-700"
        />
        <div
          className={`m-auto ms-1 bg-transparent ${
