...
function Sidebar({ menuItems = ["item1", "item2", "item3", "item4"] }) {
  ...
  return (
    ...
    >
      <span
        className={`text-center mb-2 ${expanded ? "visible" : "invisible"}`}
      >
        Sidebar Menu
      </span>
      <hr />
      <div
        className={`absolute left-[80%] md:left-[89%] top-[0%] w-8 h-8 cursor-pointer ${
      ...
      <div
        className={`absolute left-[5%] top-[1%] w-6 h-6 md:w-8 md:h-8 cursor-pointer ${
          expanded ? "invisible" : "visible"
        }`}
      >
        <img
